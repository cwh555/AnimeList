...
- Automated stress coverage performs **500 consecutive stacked raster + clipboard copies** against a file-backed temporary vault and requires the vault file list and total bytes to remain exactly unchanged. It also verifies persistent storage is never called and every decoded bitmap is closed before the next copy.
