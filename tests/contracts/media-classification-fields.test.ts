...
describe("media classification collection fields", () => {
  ...
  it("reads legacy classification headers into the canonical company, quarter, and tag view", () => {
    const frontmatter = {
      media_type: "anime",
      format: "ova",
      year: 2018,
      ...
      classification_source_genres: [
        "OVA", "戀愛", "漫畫改", "校園", "狗糧", "擅長捉弄的高木同學",
        "日常", "2018", "SHIN-EI", "2018年7月", "喜劇", "漫改",
      ],
      classification_version: 3,
      classification_genres: ["校園"],
    };
    const rows = storedMediaClassificationFieldValues(frontmatter, "anime", true);
    const values = Object.fromEntries(rows.map((row) => [row.key, row.value]));
    assert.equal(values.people, "SHIN-EI");
    assert.equal(values.season, "2018 Q3 (夏季)");
    const stored = storedMediaExternalResult(frontmatter, "anime");
    assert.deepEqual(stored.genres, ["戀愛", "日常", "喜劇", "校園"]);
    assert.equal(storedMediaNeedsClassificationRefresh(frontmatter, "anime"), false);
  });
