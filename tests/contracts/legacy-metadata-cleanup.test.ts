...
describe("legacy metadata cleanup", () => {
  ...
  it("migrates legacy classification_* headers back to main-schema headers and removes the old cache keys", () => {
    const frontmatter: Record<string, unknown> = {
      schema_version: 6,
      media_type: "anime",
      source_provider: "bangumi",
    ...
    assert.equal(change.changed, true);
    assert.deepEqual(frontmatter.genres, ["戀愛", "日常", "喜劇", "校園"]);
    assert.deepEqual(frontmatter.studios, ["SHIN-EI"]);
    assert.equal(frontmatter.season, "summer");
    assert.equal(frontmatter.season_year, 2018);
    assert.equal("classification_source_genres" in frontmatter, false);
    assert.equal("classification_version" in frontmatter, false);
