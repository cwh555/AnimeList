...
import { createReliableLibraryOpener } from "./app/library-navigation";
...
import { AnimeListSettingTab } from "./ui/settings";
import { createDefaultSettings } from "./app/settings-model";
import { AnimeListSettingsStore } from "./app/settings-store";
...
import { localizeLibraryProviderTags } from "./ui/library-tag-localization";
...
export class AnimeListPlugin extends Plugin implements AnimeListUiHost {
  ...
  renderLibrary(container: HTMLElement, items: MediaItem[], adapters: LibraryRenderAdapters = {}): void {
    ...
    prepared = {
      ...
      afterRender: (state) => {
        ...
        localizeLibraryProviderTags(container, items);
  ...
  async updateMediaNote(file: TFile, mediaType: MediaType, form: MediaNoteForm): Promise<void> {
    await this.services().updateMediaNote(file, mediaType, form);
  }
