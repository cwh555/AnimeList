...
/**
 * Verifies the configured initialization depth separately for every completion
 * date. Same-day records are the only local exception: a date with more records
 * than the configured two-sided capacity may use exactly the lanes it inherently
 * needs, but that exception cannot raise the permitted depth for another date.
 */
...
/**
 * Calculates the initial/reset spacing by validating the actual greedy lane
 * assignment. The configured depth applies to every date independently;
 * unavoidable same-day overflow remains local instead of globally relaxing the
 * limit for unrelated dense regions.
 */
