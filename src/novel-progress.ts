...
export function normalizeVolumeLog(value: unknown): NovelVolumeEntry[] {
  if (!Array.isArray(value)) return []; const entries: NovelVolumeEntry[] = []; const seen = new Set<string>();
  for (const raw of value) { const record = recordValue(raw); if (!record) continue; const label = normalizeVolumeLabel(record.label ?? record.volume); if (!label || seen.has(label)) continue; seen.add(label);
    ...
    entries.push({ label, startedAt: stringValue(record.started_at ?? record.startedAt), completedAt: stringValue(record.completed_at ?? record.completedAt), cover: stringValue(record.cover) || undefined, coverProvider: stringValue(record.cover_provider ?? record.coverProvider) || undefined, coverSourceId: stringValue(record.cover_source_id ?? record.coverSourceId) || undefined, coverManual: record.cover_manual === true || record.coverManual === true || undefined, extra: Object.keys(extra).length ? extra : undefined }); }
  return entries.sort((left, right) => compareVolumeLabels(left.label, right.label));
}
export function serializeVolumeLog(entries: NovelVolumeEntry[]): Array<Record<string, unknown>> { return normalizeVolumeLog(entries).map((entry) => { const serialized: Record<string, unknown> = { ...(entry.extra ?? {}), label: entry.label }; if (entry.startedAt) serialized.started_at = entry.startedAt; if (entry.completedAt) serialized.completed_at = entry.completedAt; if (entry.cover) serialized.cover = entry.cover; if (entry.coverProvider) serialized.cover_provider = entry.coverProvider; if (entry.coverSourceId) serialized.cover_source_id = entry.coverSourceId; if (entry.coverManual) serialized.cover_manual = true; return serialized; }); }
