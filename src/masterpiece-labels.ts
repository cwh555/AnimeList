...
export function collectMasterpieceLabels(items: Array<{ masterpieceLabels?: unknown }>): string[] {
  const labels = items.flatMap((item) => normalizeMasterpieceLabels(item.masterpieceLabels));
